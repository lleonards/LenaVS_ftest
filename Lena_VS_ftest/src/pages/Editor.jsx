...
const Editor = () => {
  ...
  // Usuário pediu sincronização automática da letra (WhisperX) → o backend
  ...
  const handleAutoSyncLyrics = useCallback(async () => {
    ...
    setSyncStatus({
      type: 'info',
      message: 'Analisando a música e a letra... isso pode levar alguns minutos.',
    });
    ...
    try {
      ...
      setSyncStatus({
        type: 'success',
        message: 'Letra sincronizada automaticamente. Revise os tempos se necessário.',
      });
    } catch (error) {
      console.error('Erro ao sincronizar letra:', error);
      const message = error.response?.data?.error || 'Não foi possível sincronizar a letra automaticamente. Tente novamente.';
