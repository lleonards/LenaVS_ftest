...
const getSafeAuthMessage = (error, context = 'login') => {
  ...
  // Falha de rede / gateway (ex.: 504 do Supabase, DNS, offline)
  const isNetworkError =
    /network error|failed to fetch|fetch failed|timeout|econnaborted|err_network/i.test(rawMessage) ||
    /50[234]/.test(String(payload.status || error?.response?.status || ''));

  if (isNetworkError) {
    return 'Servidor de autenticação indisponível neste momento. Verifique sua conexão e tente novamente em alguns instantes.';
  }
