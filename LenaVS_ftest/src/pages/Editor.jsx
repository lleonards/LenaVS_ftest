...
const Editor = () => {
  ...
  // Sincronização automática concluída → aplica início/fim em cada bloco existente.
  // O texto e a divisão dos blocos NÃO são alterados.
  const handleAlignmentProcessed = useCallback((blocks) => {
    if (!Array.isArray(blocks) || !blocks.length) return;

    const blockByIndex = new Map(blocks.map((block) => [Number(block?.index), block]));

    setStanzas((prev) =>
      prev.map((stanza, index) => {
      ...
      >
        <div className="editor-left">
          <FilesPanel
            ...
            onAlignmentProcessed={handleAlignmentProcessed}
            stanzas={stanzas}
