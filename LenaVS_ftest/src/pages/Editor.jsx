...
const Editor = () => {
  ...
  const handleAutoSync = useCallback(async () => {
    if (!mediaFiles?.musicaOriginal || !stanzas.length || syncStatus?.type === 'syncing') return;
    setSyncStatus({ type: 'syncing', message: 'Sincronizando letra… analisando o canto e os blocos existentes.' });
    try {
      const response = await api.post('/lyrics/sync', {
        audioUrl: mediaFiles.musicaOriginal,
        blocks: stanzas.map(({ id, text }) => ({ id, text })),
      });
      const assignments = response.data?.assignments || [];
      if (assignments.length !== stanzas.length) throw new Error('O detector não retornou um tempo para cada bloco.');
      setStanzas((prev) => prev.map((stanza, index) => {
        const found = assignments[index];
        if (!found || found.start == null || found.end == null) return stanza;
        return { ...stanza, startTime: formatFixedTimecode(found.start), endTime: formatFixedTimecode(found.end), hasManualStart: false, hasManualEnd: false, autoSyncConfidence: found.confidence, autoSyncMethod: found.method };
      }));
      setSyncStatus({ type: 'success', message: `Sincronização concluída: ${assignments.length} blocos preservados na ordem original.` });
    } catch (error) {
      setSyncStatus({ type: 'error', message: error.response?.data?.error || error.message || 'Não foi possível sincronizar a letra.' });
    }
  }, [mediaFiles, stanzas, syncStatus?.type]);
