...
const Editor = () => {
  ...
  const handleAutomaticSync = useCallback(async () => {
    if (isSyncing || !mediaFiles?.musicaOriginal || !stanzas.length) return;

    setIsSyncing(true);
    setSyncStatus({ type: 'info', message: 'Analisando a música cantada e procurando os blocos da letra…' });
    try {
      ...
      setStanzas((previous) => previous.map((stanza) => {
        const result = resultById.get(stanza.id);
        if (!result || result.matchedWords < 1) return stanza;
        matchedCount += 1;
        return {
          ...stanza,
          // The editor remains minute:second only; sub-second precision is
          // deliberately rounded by the existing LenaVS formatter.
          startTime: formatFixedTimecode(result.startTimeSeconds),
          endTime: formatFixedTimecode(result.endTimeSeconds),
      ...
      setSyncStatus({
        type: matchedCount === total ? 'success' : 'warning',
        message: matchedCount === total
          ? `${matchedCount} blocos sincronizados com sucesso. Você ainda pode ajustar os tempos manualmente.`
          : `${matchedCount} de ${total} blocos foram sincronizados. Os demais permaneceram como estavam; ajuste-os manualmente se necessário.`,
      });
    } catch (error) {
      console.error('Erro na sincronização automática:', error.response || error);
      setSyncStatus({
        type: 'error',
        message: error.response?.data?.error || 'Não foi possível sincronizar a letra. Os tempos existentes foram preservados.',
      });
    } finally {
      setIsSyncing(false);
    }
  }, [isSyncing, mediaFiles?.musicaOriginal, stanzas]);
      ...
      >
        <div className="editor-left">
          <FilesPanel
            ...
            onAutoSync={handleAutomaticSync}
            isSyncing={isSyncing}
          />
        </div>
