...
const Editor = () => {
  ...
  const handleSynchronizeLyrics = useCallback(async () => {
    if (!mediaFiles?.musicaOriginal || !stanzas.length) {
      throw new Error('Envie a música original e a letra antes de sincronizar.');
    }

    const response = await api.post('/lyrics/sync', {
      audioUrl: mediaFiles.musicaOriginal,
      stanzas: stanzas.map(({ id, text }) => ({ id, text })),
    });
    const syncedStanzas = Array.isArray(response.data?.stanzas)
      ? response.data.stanzas
      : [];

    if (!syncedStanzas.length) {
      throw new Error('O servidor não retornou estrofes sincronizadas.');
    }

    const analysisStanzas = Array.isArray(response.data?.analysis?.stanzas)
      ? response.data.analysis.stanzas
      : null;
    const hasVocalAnalysis = Array.isArray(analysisStanzas);

    const enrichedSyncedStanzas = syncedStanzas.map((stanza, index) => {
      const analysis = analysisStanzas?.[index] || null;
      const startTime = Number(stanza?.syncStartTime ?? stanza?.startTime);
      const endTime = Number(stanza?.syncEndTime ?? stanza?.endTime);
      const hasValidTiming = Number.isFinite(startTime)
        && Number.isFinite(endTime)
        && endTime > startTime;
      const vocalPresence = hasVocalAnalysis
        ? analysis?.vocalPresence === true
        : stanza?.vocalPresence !== false;
      const verifiedVocalMatch = hasVocalAnalysis
        ? analysis?.verified === true
        : stanza?.syncVerified !== false;
      const hasReliableVocalTiming = hasValidTiming && vocalPresence && verifiedVocalMatch;

      if (!hasReliableVocalTiming) {
        return {
          ...stanza,
          startTime: '',
          endTime: '',
          ...
          syncBlocked: true,
          syncVerified: false,
          syncConfidence: analysis?.confidence ?? stanza?.syncConfidence ?? 0,
      ...
      return {
        ...
        syncBlocked: false,
        syncVerified: analysis?.verified ?? stanza?.syncVerified ?? false,
        syncConfidence: analysis?.confidence ?? stanza?.syncConfidence ?? 0,
        syncReason: analysis?.reason || 'vocal-confirmado',
      };
    });

    const reliableSyncedCount = enrichedSyncedStanzas.filter(
      (stanza) => stanza.syncBlocked !== true
        && Number.isFinite(Number(stanza.syncStartTime))
        && Number.isFinite(Number(stanza.syncEndTime))
    ).length;

    if (reliableSyncedCount === 0) {
      throw new Error(
        'Não foi possível confirmar timestamps do vocal. A sincronização aproximada foi bloqueada para evitar letras nos trechos instrumentais.'
      );
    }

    const normalized = normalizeStanzas(enrichedSyncedStanzas, previewMetrics);
    setStanzas(normalized);
    ...
    currentTimeRef.current = 0;
    setIsPreviewPlaying(false);
    setSeekRequest(normalized[0] ? {
      stanzaId: normalized[0].id,
      time: Number(normalized[0].syncStartTime) || 0,
      nonce: Date.now(),
    } : null);
    const blockedCount = normalized.length - reliableSyncedCount;
    setSyncStatus({
      type: blockedCount > 0 ? 'warning' : 'success',
      message: blockedCount > 0
        ? `${reliableSyncedCount} estrofes sincronizadas com vocal confirmado. ${blockedCount} foram ocultadas por falta de evidência vocal.`
        : `${reliableSyncedCount} estrofes sincronizadas com o vocal da música original.`,
    });
    setIsLibraryForkNoEdits(false);
  }, [mediaFiles?.musicaOriginal, previewMetrics, stanzas]);
      ...
      prev.map((stanza) =>
        stanza.id === stanzaId
          ? {
              ...
              ...(field === 'startTime' ? { syncStartTime: null } : { syncEndTime: null }),
              [flagField]: true,
              showOnlyDuringVocal: false,
              syncBlocked: false,
            }
          : stanza
      )
      ...
      >
        <div className="editor-left">
          <FilesPanel
            ...
            onSynchronizeLyrics={handleSynchronizeLyrics}
