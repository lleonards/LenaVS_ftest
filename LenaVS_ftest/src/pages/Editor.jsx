...
const Editor = () => {
  ...
  const handleAutoSync = useCallback(async () => {
    ...
    const audioSource = mediaMetadata?.musicaOriginal?.storagePath
      || mediaMetadata?.musicaOriginal?.publicUrl
    ...
    if (!audioSource) {
      setSyncStatus({
        type: 'warning',
        message: 'Envie a música original antes de iniciar a sincronização automática.',
      });
      return;
    }
    ...
    if (!stanzas.length) {
      setSyncStatus({
        type: 'warning',
        message: 'Processe a letra para criar os blocos antes da sincronização automática.',
      });
      return;
    }
    ...
    try {
      ...
      setSyncStatus({
        type: 'info',
        message: 'Sincronizando letra... analisando a música original e os blocos existentes.',
      });
      ...
      const timings = Array.isArray(response.data?.timings) ? response.data.timings : [];
      if (!timings.length || timings.length !== stanzas.length) {
        throw new Error('O servidor não retornou tempos válidos para todos os blocos.');
      }

      const timingsById = new Map(timings.map((item) => [item.id, item]));

      setStanzas((prev) => prev.map((stanza, index) => {
        const timing = timingsById.get(stanza.id) || timings[index];
        if (!timing) return stanza;
        ...
        return {
          ...
          hasManualEnd: false,
          showOnlyDuringVocal: timing.showOnlyDuringVocal !== false,
          syncConfidence: timing.syncConfidence,
          syncVerified: timing.syncVerified,
          syncLowConfidence: timing.syncLowConfidence,
          vocalPresence: timing.vocalPresence,
          speechActivityDetected: timing.speechActivityDetected,
          syncSource: timing.syncSource,
          syncEngine: timing.syncEngine,
          syncReason: timing.syncReason,
        };
      }));

      const summaryMessage = response.data?.summary?.message || 'Sincronização concluída com sucesso.';
      setSyncStatus({
        type: response.data?.summary?.type || 'success',
        message: summaryMessage,
      });
      pushNotice({
        type: response.data?.summary?.type || 'success',
        title: 'Sincronização automática concluída',
        message: `${summaryMessage} Você ainda pode ajustar manualmente qualquer bloco no editor.`,
      });
    } catch (error) {
      console.error('Erro ao sincronizar automaticamente a letra:', error);
      const message = error.response?.data?.error || error.message || 'Não foi possível sincronizar a letra automaticamente.';
      setSyncStatus({
        type: 'error',
        message,
      });
  ...
  }, [isAutoSyncing, mediaFiles, mediaMetadata, pushNotice, stanzas]);

  const secondsToTimecode = (seconds) => formatFixedTimecode(seconds, { decimals: 2 });
