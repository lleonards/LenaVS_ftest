...
const Editor = () => {
  ...
  // Sincronização automática (ctc-forced-aligner 1.0.2): preenche apenas os
  // TEMPOS dos blocos existentes — texto, ordem e divisão dos blocos ficam
  // exatamente como o usuário criou.
  const handleSyncApplied = useCallback((timings) => {
    setStanzas((prev) =>
      prev.map((stanza) => {
        const match = timings.find((timing) => timing.id === stanza.id);
