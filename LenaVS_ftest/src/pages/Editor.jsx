...
const Editor = () => {
  ...
  // Sincronização automática aplicada → atualiza apenas os tempos dos blocos
  const handleStanzasSynced = useCallback((nextStanzas, result) => {
    const normalized = normalizeStanzas(nextStanzas, previewMetrics);
    setStanzas(normalized);
    setShowLyricsEditor(normalized.length > 0);

    const syncedCount = result?.stats?.syncedBlocks ?? normalized.length;
    const totalCount = result?.stats?.blocks ?? normalized.length;

    setSyncStatus({
      type: 'success',
      message: `Sincronização automática concluída: ${syncedCount}/${totalCount} blocos sincronizados.`,
    });
  }, [previewMetrics]);
