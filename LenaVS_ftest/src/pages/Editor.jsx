...
const Editor = () => {
  ...
  const handleAutoSyncLyrics = useCallback(async () => {
    ...
    if (!audioUrl || !stanzas.length || isSyncingLyrics) return;
    ...
    try {
      ...
      setStanzas((prev) => prev.map((stanza) => {
        const match = timed.find((item) => item.id === stanza.id);
      ...
      const message = error.response?.status === 503
        ? 'O servidor está ocupado protegendo os recursos da máquina. Aguarde alguns instantes e tente a sincronização novamente.'
        : error.response?.data?.error
          || '🎵 Ops! Não foi possível concluir a sincronização automática agora. Tente novamente ou ajuste os tempos manualmente.';
