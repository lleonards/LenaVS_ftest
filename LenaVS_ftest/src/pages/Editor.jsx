...
const Editor = () => {
  ...
  // Sincronização automática: aplica os tempos (mm:ss) aos blocos EXATOS da LenaVS
  const handleLyricsSyncApplied = useCallback((blocks) => {
    const safeBlocks = Array.isArray(blocks) ? blocks : [];
    const matchedCount = safeBlocks.filter((block) => block.matched).length;

    setStanzas((prev) => {
      if (safeBlocks.length === 0 || prev.length === 0) return prev;
      const byIndex = new Map(safeBlocks.map((block, index) => [index, block]));

      return prev.map((stanza, index) => {
        const aligned = byIndex.get(index);
        if (!aligned || aligned.startTime == null || aligned.endTime == null) {
          return stanza;
        }

        // A letra NÃO é reorganizada: apenas os tempos de início/fim são aplicados.
        // Bloco.início = 1ª palavra cantada; Bloco.fim = última palavra cantada.
        return {
          ...stanza,
          startTime: formatFixedTimecode(aligned.startTime),
          endTime: formatFixedTimecode(aligned.endTime),
    ...
    setSyncStatus({
      ...
      message: matchedCount > 0
        ? `Letra sincronizada automaticamente: ${matchedCount} de ${safeBlocks.length} blocos alinhados com o áudio.`
        : 'Não foi possível alinhar os blocos com o áudio. Confira se a letra corresponde à música.',
    });
  }, []);
      ...
      >
        <div className="editor-left">
          <FilesPanel
            ...
            stanzas={stanzas}
            onStanzasSyncApplied={handleLyricsSyncApplied}
