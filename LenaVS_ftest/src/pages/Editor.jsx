...
const Editor = () => {
  ...
  const handleAutoSync = useCallback(async () => {
    if (!mediaFiles?.musicaOriginal || !stanzas.length || syncStatus?.phase === 'syncing') return;
    setSyncStatus({ phase: 'syncing', type: 'info', message: 'Sincronizando letra… analisando o áudio e preservando os blocos da LenaVS.' });
    try {
      const response = await api.post('/vocal-sync', { audioUrl: mediaFiles.musicaOriginal, blocks: stanzas.map(({ id, text }) => ({ id, text })) });
      const assignments = response.data?.blocks || [];
      if (assignments.length !== stanzas.length || assignments.some((b, i) => b.id !== stanzas[i].id)) throw new Error('O detector retornou estrutura diferente. Nenhum bloco foi alterado.');
      const byId = new Map(assignments.map((b) => [b.id, b]));
      setStanzas((prev) => prev.map((stanza) => { const hit = byId.get(stanza.id); if (!hit || hit.start === null || hit.end === null) return stanza; return { ...stanza, startTime: secondsToTimecode(hit.start), endTime: secondsToTimecode(hit.end), hasManualStart: false, hasManualEnd: false, autoSyncConfidence: hit.confidence, autoSyncRegionIndex: hit.vocalRegionIndex }; }));
      setSyncStatus({ phase: 'done', type: 'success', message: `Sincronização concluída: ${assignments.length} blocos preservados.` });
    } catch (error) { setSyncStatus({ phase: 'error', type: 'error', message: error.response?.data?.error || error.message || 'Não foi possível sincronizar o áudio.' }); }
  }, [mediaFiles, stanzas, syncStatus]);
