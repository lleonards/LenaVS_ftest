...
export const hasDefinedStartTime = (stanza, options = {}) => {
  if (Number.isFinite(Number(stanza?.syncStartTime))) {
    return true;
  }
...
export const hasDefinedEndTime = (stanza) => {
  if (Number.isFinite(Number(stanza?.syncEndTime))) {
    return true;
  }
...
export const hasConfiguredTiming = (stanza) => {
  if (
    ...
    && Number(stanza.syncEndTime) >= Number(stanza.syncStartTime)
  ) {
    return true;
  }
...
/**
 * Sincronizações automáticas marcadas como "apenas durante o vocal" precisam
 * ter evidência de vocal. O campo é deliberadamente tri-state para manter
 * compatibilidade com projetos antigos que não possuem análise de vocal:
 * - false: o backend confirmou que não havia vocal naquele match;
 * - true: existe evidência vocal;
 * - ausente: projeto legado/manual, segue as regras antigas.
 */
export const hasVocalEvidence = (stanza = {}) => {
  // Marcado explicitamente como bloqueado pela sincronia automática => esconde estrofes.
  if (stanza?.syncBlocked === true) {
    return false;
  }

  // Backend reportou explicitamente ausência de vocal em qualquer um dos flags.
  if (stanza?.vocalPresence === false || stanza?.syncVocalPresence === false) {
    return false;
  }

  // Quando o usuário pediu "apenas durante o vocal" e o backend ainda não se pronunciou
  // (null / undefined) mantemos o comportamento legado (assume que tem vocal).
  if (stanza?.showOnlyDuringVocal !== true) {
    return true;
  }

  return stanza?.vocalPresence !== false
    && stanza?.syncVocalPresence !== false;
};
...
const resolveStanzaEndTime = (stanzas = [], index, start) => {
  ...
  const explicitEnd = Number.isFinite(Number(stanza?.syncEndTime))
  ...
  for (let nextIndex = index + 1; nextIndex < stanzas.length; nextIndex += 1) {
    ...
    const nextStart = Number.isFinite(Number(nextStanza?.syncStartTime))
      ? Number(nextStanza.syncStartTime)
      : parseFixedTimecode(nextStanza?.startTime);
...
export const getResolvedStanzaTimeline = (stanzas = []) => {
  ...
  return safeStanzas.map((stanza, index) => {
    ...
    if (!hasVocalEvidence(stanza) || !hasDefinedStartTime(stanza, { allowFirstBlockAtZero })) {
      return null;
    }
