...
api.interceptors.response.use(
  (response) => response,
  async (error) => {
    // Marca como erro esperado quando é um 422 do /api/lyrics/sync — esse status
    // significa apenas que o backend não encontrou vocal, e o frontend já sabe lidar.
    try {
      const status = error.response?.status;
      const url = error.config?.url || '';
      if (status === 422 && url.includes('/lyrics/sync')) {
        if (!error.expected) error.expected = 'no-vocal';
      }
    } catch { /* noop */ }
