...
const FilesPanel = ({
  ...
  const isSyncing = syncStatus?.type === 'syncing';
  ...
  const anyBusy = isUploading || isCreatingInstrumental || isSyncing;

  // Instrumental pode ser criado quando há música original mas ainda não há instrumental
  const canCreateInstrumental = Boolean(mediaFiles?.musicaOriginal) && !isCreatingInstrumental && !isSyncing;
  const canAutoSync = Boolean(mediaFiles?.musicaOriginal) && Number(stanzaCount) > 0 && !isSyncing && !isCreatingInstrumental;

  const handleAutoSync = async () => {
    if (!canAutoSync || typeof onAutoSync !== 'function') return;
    await onAutoSync();
  };
      ...
      <div className="auto-sync-section auto-sync-section--lyrics">
        <div className="auto-sync-wrapper">
          <button
            ...
            title={canAutoSync ? 'Analisar a música e localizar os blocos existentes da letra' : 'Envie a música original e processe a letra para habilitar'}
          >
            <span className="auto-sync-btn-content">
              <span className="auto-sync-btn-main">
                {isSyncing ? <Loader2 size={15} className="spin-icon" /> : <Music2 size={15} />}
                <span>{isSyncing ? 'Sincronizando letra…' : 'Sincronizar automaticamente'}</span>
              </span>
              <span className="auto-sync-btn-note">Usa os blocos atuais sem reorganizar</span>
            </span>
          </button>
        </div>
        <p className="auto-sync-hint">
          {isSyncing ? 'Analisando canto, pausas e regiões instrumentais…' : !mediaFiles?.musicaOriginal ? 'Faça upload da música original para habilitar.' : !stanzaCount ? 'Processe a letra para criar os blocos.' : 'Atribui tempos reais aos blocos já criados pela LenaVS.'}
        </p>
      </div>
