...
import { Upload, Type, X, Loader2, Sparkles, Music2, Wand2 } from 'lucide-react';
import api from '../services/api';
import { useAuth } from '../contexts/AuthContext';
import './FilesPanel.css';
import { createStanzaFromText } from '../utils/stanza';
import { describeSyncError } from '../utils/useLyricsSync';
import {
...
const getFriendlyApiError = (error, file, fallback) => {
  ...
  if (responseData.code === 'DEMUCS_UNAVAILABLE') {
    return 'O separador de voz não está instalado no servidor. Verifique o ambiente Python do backend e tente novamente.';
  }

  if (responseData.code === 'DEMUCS_TIMEOUT' || error?.response?.status === 504) {
    return 'A separação demorou mais do que o limite do servidor. Tente um áudio menor.';
  }

  if (error?.response?.status === 507) {
    return 'O servidor ficou sem memória ao criar o instrumental. Tente um áudio menor ou em MP3.';
  }
...
const FilesPanel = ({
  ...
  // O backend serializa tarefas pesadas porque este deployment tem 1 CPU /
  // 2 GB. Isso é apenas uma proteção de recurso: instrumental e sincronização
  // continuam sendo fluxos independentes, com endpoints e dados separados.
  const anyBusy = isUploading || isCreatingInstrumental || isSyncingLyrics;
  ...
  // Sincronização automática: só habilita com a música original E a letra enviadas.
  const canAutoSyncLyrics = Boolean(mediaFiles?.musicaOriginal) && stanzaCount > 0;
  ...
  const handleCreateInstrumental = async () => {
    ...
    try {
      ...
      const { instrumentalUrl, instrumentalMetadata, duration } = response.data || {};
      ...
      // Atualiza os metadados (URL + bucket + storagePath) para que a URL
      // assinada possa ser renovada depois sem quebrar o áudio.
      onMediaMetadataChange?.({
        musicaInstrumental: normalizeUploadedMediaMetadataEntry({
          ...(mediaMetadata?.musicaInstrumental || {}),
          kind: 'audio',
          duration: duration ?? mediaMetadata?.musicaInstrumental?.duration ?? null,
          publicUrl: instrumentalMetadata?.publicUrl ?? instrumentalUrl,
          storagePath: instrumentalMetadata?.storagePath ?? null,
          bucket: instrumentalMetadata?.bucket ?? null,
        }, 'audio'),
      });
  ...
  const handleAutoSyncLyrics = async () => {
    if (!canAutoSyncLyrics || anyBusy || isSyncingLyrics || !isAuthenticated) return;
    if (typeof onAutoSyncLyrics !== 'function') return;
    ...
    try {
      ...
      onNotify?.({
        type: 'info',
        title: 'Sincronizando letra…',
        message: 'Analisando a música e descobrindo o tempo de cada palavra. Isso pode levar alguns minutos.',
      });

      const result = await onAutoSyncLyrics();

      onNotify?.({
        type: 'success',
        title: 'Letra sincronizada!',
        message: result?.estimated
          ? `Os tempos de ${result?.applied ?? stanzaCount} bloco(s) foram definidos por estimativa econômica.`
          : `Início e fim de ${result?.applied ?? stanzaCount} bloco(s) foram definidos automaticamente.`,
      });
    } catch (error) {
      console.error('[FilesPanel] Erro ao sincronizar letra:', error);
  ...
  {isCreatingInstrumental
    ...
    : !mediaFiles?.musicaOriginal
      ...
      <div className="auto-sync-section auto-sync-section--lyrics">
        <div className="auto-sync-wrapper">
          <button
            type="button"
            className={`auto-sync-btn ${canAutoSyncLyrics ? 'unlocked' : 'locked'}`}
            onClick={handleAutoSyncLyrics}
            disabled={!canAutoSyncLyrics || anyBusy || isSyncingLyrics}
            title={
              !mediaFiles?.musicaOriginal
                ? 'Faça o upload da música original primeiro'
                : stanzaCount === 0
                  ? 'Envie a letra primeiro'
                  : 'Descobrir automaticamente o tempo de cada palavra da letra'
            }
          >
            {isSyncingLyrics ? (
              <span className="auto-sync-btn-content">
                ...
                <span className="auto-sync-btn-note">Analisando a música e a letra</span>
              </span>
            ) : (
              <span className="auto-sync-btn-content">
                <span className="auto-sync-btn-main">
                ...
                <span className="auto-sync-btn-note">Descobre o tempo de cada palavra</span>
              </span>
            )}
          </button>
        </div>
        ...
        <p className="auto-sync-hint">
          {isSyncingLyrics
            ? 'Encontrando o tempo de cada palavra da letra…'
            : !mediaFiles?.musicaOriginal
              ? 'Faça upload da música original e da letra para habilitar.'
              : stanzaCount === 0
                ? 'Envie a letra para habilitar.'
                : 'Define o início e o fim de cada bloco automaticamente.'}
        </p>

        <p className="auto-sync-disclaimer">Este recurso pode não funcionar 100%.</p>
      </div>
