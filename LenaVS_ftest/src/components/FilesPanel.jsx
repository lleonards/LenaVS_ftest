...
const FilesPanel = ({
  ...
  const anyBusy = isUploading || isCreatingInstrumental || isAutoSyncing;
  ...
  // Sincronização automática: só libera quando há música original E letra (blocos)
  const canAutoSync = Boolean(mediaFiles?.musicaOriginal) && stanzaCount > 0;
  ...
  const handleAutoSync = async () => {
    if (!canAutoSync || anyBusy || !isAuthenticated) return;

    const blocks = (Array.isArray(stanzas) ? stanzas : [])
      .map((stanza) => ({ id: stanza?.id, text: String(stanza?.text ?? '') }))
      .filter((stanza) => stanza.id && stanza.text.trim().length > 0);

    if (!blocks.length) {
      onNotify?.({
        type: 'warning',
        title: 'Letra não encontrada',
        message: 'Envie a letra (arquivo ou colada) antes de sincronizar.',
      });
      return;
    }
    ...
    try {
      ...
      onNotify?.({
        ...
        message: 'Detectando as palavras no áudio e calculando o início e o fim de cada bloco.',
      });
      ...
      const synced = Array.isArray(response.data?.stanzas) ? response.data.stanzas : [];
      const syncedById = new Map(synced.map((entry) => [entry.id, entry]));

      // Aplica início/fim por bloco sem tocar no texto nem na ordem dos blocos
      const nextStanzas = stanzas.map((stanza) => {
        const match = syncedById.get(stanza.id);
      ...
      onStanzasSynced?.(nextStanzas, response.data);
    } catch (error) {
      console.error('[FilesPanel] Erro na sincronização automática:', error.response || error);
      onNotify?.({
        type: 'error',
        title: 'Falha na sincronização automática',
        message: error.response?.data?.error || error.message || 'Não foi possível sincronizar a letra.',
      });
    } finally {
    ...
    : !mediaFiles?.musicaOriginal
      ...
      <div className="auto-sync-section">
        <div className="auto-sync-wrapper">
          <button
            type="button"
            className={`auto-sync-btn auto-sync-btn--lyrics ${canAutoSync ? 'unlocked' : 'locked'}`}
            ...
            title={
              !mediaFiles?.musicaOriginal
                ? 'Faça o upload da música original primeiro'
                : stanzaCount === 0
                  ? 'Envie a letra (arquivo ou colada) primeiro'
                  : 'Detectar as palavras no áudio e definir início/fim de cada bloco da letra'
            }
          >
            {isAutoSyncing ? (
              <span className="auto-sync-btn-content">
                ...
                <span className="auto-sync-btn-note">este recurso pode apresentar falhas</span>
              </span>
            ) : (
              <span className="auto-sync-btn-content">
                <span className="auto-sync-btn-main">
        ...
        <p className="auto-sync-hint">
          {isAutoSyncing
            ? 'Ouvindo a música e calculando o tempo de cada bloco...'
            : !mediaFiles?.musicaOriginal
              ? 'Faça upload da música original para habilitar.'
              : stanzaCount === 0
                ? 'Envie a letra para habilitar.'
                : 'Define início e fim de cada bloco da letra automaticamente.'}
        </p>
      </div>
