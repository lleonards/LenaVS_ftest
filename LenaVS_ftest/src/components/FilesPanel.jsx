...
import { Upload, Type, X, Loader2, Sparkles, Music2, RefreshCw } from 'lucide-react';
...
const FilesPanel = ({
  ...
  // Sincronização automática: exige música original + letra carregada
  const canSyncLyrics = Boolean(
    mediaFiles?.musicaOriginal
    && Array.isArray(onSelectedStanzas)
    && onSelectedStanzas.length > 0
  ) && !isSyncingLyrics;
  ...
  const handleSyncLyrics = async () => {
    ...
    const stanzasToSync = Array.isArray(onSelectedStanzas) ? onSelectedStanzas : [];
    if (stanzasToSync.length === 0) return;
    ...
    try {
      ...
      onNotify?.({
        ...
        message: 'Analisando a música para encontrar o tempo de cada palavra. Isso pode levar um ou dois minutos.',
      });

      const response = await api.post('/lyrics/sync', {
        audioUrl: mediaFiles.musicaOriginal,
        blocks: stanzasToSync.map((stanza) => ({ text: stanza?.text || '' })),
      });

      const { timings = [] } = response.data || {};

      if (!timings.length || timings.length !== stanzasToSync.length) {
        throw new Error('O servidor não retornou os tempos para todos os blocos.');
      }

      // Preenche apenas os TEMPOS dos blocos existentes (mm:ss, sem milissegundos)
      onSyncApplied?.(timings.map((timing, index) => ({
        id: stanzasToSync[index].id,
        start: timing.start,
        end: timing.end,
      })));
      ...
      onNotify?.({
        ...
        message: `${timings.length} bloco(s) sincronizado(s) automaticamente com a música.`,
      });
    } catch (error) {
      ...
      onNotify?.({
        ...
        message: error.response?.data?.error || error.message || 'Ocorreu um erro ao sincronizar a letra. A letra e os blocos permanecem intactos.',
      });
    } finally {
  ...
  {isCreatingInstrumental
    ...
    : !mediaFiles?.musicaOriginal
      ...
      {/* ── Sincronizar Letra Automaticamente ─────────────────────────── */}
      <div className="auto-sync-section">
        <div className="auto-sync-wrapper">
          <button
            type="button"
            ...
            title={
              !mediaFiles?.musicaOriginal
                ? 'Faça o upload da música original primeiro'
                : Array.isArray(onSelectedStanzas) && onSelectedStanzas.length === 0
                  ? 'Adicione a letra antes de sincronizar'
                  : 'Analisa a música e preenche automaticamente os tempos dos blocos existentes'
            }
          >
            {isSyncingLyrics ? (
              <span className="auto-sync-btn-content">
        ...
        <p className="auto-sync-hint">
          {isSyncingLyrics
            ? 'Analisando a música… seus blocos e letra não são alterados em caso de falha.'
            : 'Este recurso pode não ser 100% funcional'}
        </p>
      </div>
