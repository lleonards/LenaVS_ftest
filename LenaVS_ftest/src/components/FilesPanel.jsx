...
const FilesPanel = ({
  ...
  // Sincronização automática: só fica disponível com a música E a letra enviadas.
  const hasLyricsReady = Boolean(stanzas?.length) || stanzaCount > 0;
  const canSyncLyrics = Boolean(mediaFiles?.musicaOriginal) && hasLyricsReady && !isSyncingLyrics;
  ...
  const handleAutoSyncLyrics = async () => {
    if (!canSyncLyrics || anyBusy || !isAuthenticated) return;
    ...
    try {
      ...
      onNotify?.({
        ...
        message: 'Analisando a música e a letra para descobrir o tempo de cada palavra. Isso pode levar alguns minutos.',
      });
      ...
      const response = await api.post(
        ...
        {
          audioUrl: mediaFiles.musicaOriginal,
          stanzas: (stanzas || []).map((stanza) => ({ text: stanza?.text ?? '' })),
        },
        { timeout: 30 * 60 * 1000 }
      );
      ...
      onAlignmentProcessed?.(blocks);

      const timedCount = blocks.filter((block) => block?.startTime && block?.endTime).length;
      const isApproximate = response.data?.meta?.reliable === false;

      onNotify?.({
        type: isApproximate ? 'warning' : 'success',
        title: isApproximate ? 'Sincronização concluída (aproximada)' : 'Letra sincronizada!',
        message: isApproximate
          ? `${timedCount} blocos receberam tempo automático. Este recurso pode não funcionar 100% — revise os tempos no Editor de Letras.`
          : `${timedCount} blocos receberam início e fim automáticos. Confira no Editor de Letras.`,
      });
    } catch (error) {
      ...
      onNotify?.({
        ...
        message: status === 429
          ...
          : serverMessage || error.message || 'Não foi possível sincronizar a letra automaticamente.',
      });
    } finally {
  ...
  {isCreatingInstrumental
    ...
    : !mediaFiles?.musicaOriginal
      ...
      <div className="auto-sync-section auto-sync-section--alignment">
        <div className="auto-sync-wrapper">
          <button
            type="button"
            className={`auto-sync-btn ${canSyncLyrics ? 'unlocked' : 'locked'}`}
            onClick={handleAutoSyncLyrics}
            disabled={!canSyncLyrics || anyBusy}
            title={
              !mediaFiles?.musicaOriginal
                ? 'Faça o upload da música original primeiro'
                : !hasLyricsReady
                  ? 'Envie a letra (arquivo ou colar) primeiro'
                  : 'Analisa a música e a letra e descobre o tempo de cada palavra'
            }
          >
            {isSyncingLyrics ? (
              <span className="auto-sync-btn-content">
              ...
              <span className="auto-sync-btn-content">
                ...
                <span className="auto-sync-btn-note">Usa a música e a letra enviadas</span>
              </span>
            )}
          </button>
        </div>
        ...
        <p className="auto-sync-hint">
          {isSyncingLyrics
            ? 'Analisando a música e descobrindo o tempo de cada palavra…'
            : !mediaFiles?.musicaOriginal
              ? 'Faça upload da música original para habilitar.'
              : !hasLyricsReady
                ? 'Envie a letra para habilitar.'
                : 'Descobre o tempo de cada palavra e preenche início e fim dos blocos.'}
        </p>
      </div>
