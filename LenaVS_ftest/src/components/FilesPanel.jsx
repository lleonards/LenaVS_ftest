...
import { Upload, Type, X, Loader2, Sparkles, Music2, Waves } from 'lucide-react';
...
const FilesPanel = ({
  ...
  // Sincronização automática: só disponível com música original + letra (blocos) no projeto
  const canSyncLyrics = Boolean(mediaFiles?.musicaOriginal) && stanzaCount > 0 && !isSyncingLyrics;
  ...
  const handleSyncLyrics = async () => {
    ...
    try {
      ...
      onNotify?.({
        ...
        message: 'Detectando o canto e alinhando a letra palavra por palavra. Pode levar alguns instantes.',
      });
      ...
      if (!Array.isArray(blocks) || blocks.length === 0) {
        throw new Error('O servidor não retornou blocos sincronizados.');
      }

      const matchedCount = blocks.filter((block) => block.matched).length;
      const totalCount = blocks.length;

      // Aplica os tempos (mm:ss) aos blocos EXATOS da LenaVS, sem reorganizar nada
      onStanzasSyncApplied?.(blocks);

      onNotify?.({
        type: matchedCount > 0 ? 'success' : 'warning',
        title: matchedCount > 0 ? 'Letra sincronizada!' : 'Sincronização parcial',
        message: matchedCount > 0
          ? `${matchedCount} de ${totalCount} blocos alinhados com o áudio.`
          : 'Não foi possível alinhar os blocos. Verifique se o áudio tem voz e se a letra corresponde à música.',
      });
    } catch (error) {
      console.error('[FilesPanel] Erro ao sincronizar letra:', error.response || error);
      onNotify?.({
        type: 'error',
        title: 'Falha ao sincronizar letra',
        message: error.response?.data?.error || error.message || 'Erro ao sincronizar a letra.',
      });
    } finally {
  ...
  {isCreatingInstrumental
    ...
    : !mediaFiles?.musicaOriginal
      ...
      <div className="auto-sync-section">
          ...
          <button
            ...
            title={
              !mediaFiles?.musicaOriginal
                ? 'Faça o upload da música original primeiro'
                : stanzaCount === 0
                  ? 'Adicione a letra para sincronizar'
                  : 'Alinha cada bloco da letra no momento exato em que ele é cantado'
            }
          >
            {isSyncingLyrics ? (
              <span className="auto-sync-btn-content">
                ...
                <span className="auto-sync-btn-note">Alinhando palavra por palavra</span>
              </span>
            ) : (
              <span className="auto-sync-btn-content">
                <span className="auto-sync-btn-main">
                ...
                <span className="auto-sync-btn-note">Alinha os blocos com o áudio</span>
              </span>
            )}
          </button>
        </div>
        ...
        <p className="auto-sync-hint">
          {isSyncingLyrics
            ? 'Detectando o canto e alinhando a letra… isso pode levar alguns instantes.'
            : !mediaFiles?.musicaOriginal
              ? 'Faça upload da música original para habilitar.'
              : stanzaCount === 0
                ? 'Adicione a letra para habilitar.'
                : 'Sincroniza cada bloco no momento em que ele é cantado.'}
        </p>
      </div>
