...
const FilesPanel = ({
  ...
  const canAutoSync = Boolean(mediaFiles?.musicaOriginal) && stanzaCount > 0 && !isSyncing;
  ...
  {isCreatingInstrumental
    ...
    : !mediaFiles?.musicaOriginal
      ...
      <div className="auto-sync-section auto-sync-section--lyrics">
          ...
          <button
            ...
            title={
              !mediaFiles?.musicaOriginal
                ...
                : !stanzaCount
                  ? 'Adicione uma letra primeiro'
                  : 'Analisar a música e preencher os tempos dos blocos'
            }
          >
            <span className="auto-sync-btn-content">
              <span className="auto-sync-btn-main">
                {isSyncing ? <Loader2 size={15} className="spin-icon" /> : <Music2 size={15} />}
                <span>{isSyncing ? 'Sincronizando…' : 'Sincronizar automaticamente'}</span>
              </span>
              <span className="auto-sync-btn-note">
                {isSyncing ? 'Analisando a música cantada' : 'Preenche os tempos dos blocos da letra'}
              </span>
            </span>
          </button>
        </div>
        <p className="auto-sync-hint">
          {!mediaFiles?.musicaOriginal
            ? 'Faça upload da música original para habilitar.'
            : !stanzaCount
              ? 'Adicione uma letra para habilitar.'
              : 'A letra continua organizada nos mesmos blocos.'}
        </p>
      </div>
      ...
      {syncStatus && (syncStatus.type !== 'info' || isSyncing) ? (
