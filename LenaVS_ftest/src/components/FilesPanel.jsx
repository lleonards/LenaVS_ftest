...
const FilesPanel = ({
  ...
  const canAutoSync = Boolean(mediaFiles?.musicaOriginal) && stanzaCount > 0 && !isAutoSyncing;
  ...
  return (
    <div className="files-panel" ref={panelRef}>
      ...
      {/* ── Criar Instrumental com IA + Sincronização automática ───────────────── */}
      <div className="auto-sync-section">
        ...
        <button
          ...
          title={
            !mediaFiles?.musicaOriginal
              ? 'Faça o upload da música original para habilitar a sincronização automática'
              : stanzaCount <= 0
                ? 'Processe a letra para criar os blocos antes de sincronizar'
                : 'Analisar a música original e localizar quando cada bloco é cantado'
          }
        >
          {isAutoSyncing ? (
            <span className="auto-sync-btn-content">
              ...
              <span className="auto-sync-btn-note">Analisando o canto e os blocos existentes</span>
            </span>
          ) : (
            <span className="auto-sync-btn-content">
              <span className="auto-sync-btn-main">
              ...
              <span className="auto-sync-btn-note">Usa a música original + a letra já processada</span>
            </span>
          )}
        </button>
        ...
        <p className="auto-sync-hint">
          {isAutoSyncing
            ? 'Sincronizando letra... aguarde a análise terminar.'
            : !mediaFiles?.musicaOriginal && stanzaCount <= 0
              ? 'Envie a música original e processe a letra para habilitar.'
              : !mediaFiles?.musicaOriginal
                ? 'Faça upload da música original para habilitar.'
                : stanzaCount <= 0
                  ? 'Processe a letra para criar os blocos antes da sincronização automática.'
                  : 'A sincronização respeita exatamente os blocos já criados pela LenaVS e você poderá revisar manualmente depois.'}
        </p>
      </div>
