...
const FilesPanel = ({
  ...
  const [isSynchronizingLyrics, setIsSynchronizingLyrics] = useState(false);
  ...
  const anyBusy = isUploading || isCreatingInstrumental || isSynchronizingLyrics;
  ...
  const canSynchronizeLyrics = Boolean(
    mediaFiles?.musicaOriginal
    && stanzaCount > 0
    && !isSynchronizingLyrics
  );
  ...
  /**
   * Extrai uma mensagem amigável do erro de sincronização,
   * cobrindo os três cenários reais:
   *  - 422 com vocal não detectado (antigo bug do backend legado)
   *  - 4xx de validação (URL inválida, etc.)
   *  - 5xx ou erro de rede
   */
  const buildSyncErrorMessage = (error) => {
    ...
    if (status === 422 || code === 'VOCAL_TIMESTAMPS_NOT_FOUND') {
      return 'Não foi possível detectar vocal na música. As estrofes ficarão invisíveis nas partes sem canto.';
    }
    if (status === 400 && code === 'INVALID_AUDIO_URL') {
      return 'A música original precisa estar disponível antes de sincronizar.';
    }
    if (status === 413 || code === 'AUDIO_TOO_LONG') {
      return apiMessage || 'O áudio enviado excede o limite. Reduza a duração e tente novamente.';
    }
    if (status && status >= 500) {
      return 'Erro temporário no servidor durante a sincronização. Tente novamente em alguns instantes.';
    }
    if (apiMessage) return apiMessage;
    return error?.message || 'Não foi possível sincronizar a letra com o vocal.';
  };

  const handleSynchronizeLyrics = async () => {
    if (!canSynchronizeLyrics || anyBusy || !isAuthenticated) return;

    try {
      setIsSynchronizingLyrics(true);
      onNotify?.({
        type: 'info',
        title: 'Sincronizando letra…',
        message: 'Analisando o vocal da música original e ajustando cada estrofe.',
      });

      await onSynchronizeLyrics?.();
    } catch (error) {
      // O backend v2 devolve HTTP 200 mesmo sem vocal — então este catch só dispara em
      // erros verdadeiros (timeout, 5xx, rede). O caso "sem vocal" não passa por aqui.
      const status = error?.response?.status;
      if (status === 422) {
        // Segurança extra caso um backend legado ainda responda 422: tratamos como aviso,
        // não como erro fatal, e seguimos.
        onNotify?.({
          type: 'warning',
          title: 'Vocal não detectado',
          message: buildSyncErrorMessage(error),
        });
      } else {
        onNotify?.({
          type: 'error',
          title: 'Falha ao sincronizar letra',
          message: buildSyncErrorMessage(error),
        });
      }
    } finally {
      setIsSynchronizingLyrics(false);
    }
  };
  ...
  return (
    <div className="files-panel" ref={panelRef}>
      ...
      <div className="auto-sync-section">
        ...
        <button
          type="button"
          className={`auto-sync-btn auto-sync-btn--lyrics ${canSynchronizeLyrics ? 'unlocked' : 'locked'}`}
          onClick={handleSynchronizeLyrics}
          disabled={!canSynchronizeLyrics || anyBusy}
          title={
            !mediaFiles?.musicaOriginal
              ? 'Envie a música original primeiro'
              : stanzaCount === 0
                ? 'Envie ou cole a letra primeiro'
                : 'Sincronizar as estrofes com o vocal da música original'
          }
        >
          {isSynchronizingLyrics ? (
            <span className="auto-sync-btn-content">
              <span className="auto-sync-btn-main">
                <Loader2 size={15} className="spin-icon" />
                <span>Sincronizando letra…</span>
              </span>
              <span className="auto-sync-btn-note">Analisando o vocal da música original</span>
            </span>
          ) : (
            <span className="auto-sync-btn-content">
              <span className="auto-sync-btn-main">
              ...
              <span className="auto-sync-btn-note">Mostra cada estrofe somente quando o vocal canta</span>
            </span>
          )}
        </button>
        ...
        <p className="auto-sync-hint">
          {!mediaFiles?.musicaOriginal
            ? 'Envie a música original para habilitar.'
            : stanzaCount === 0
              ? 'Envie ou cole a letra para habilitar.'
              : 'Sincroniza a letra com os momentos reais do vocal.'}
        </p>
      </div>
