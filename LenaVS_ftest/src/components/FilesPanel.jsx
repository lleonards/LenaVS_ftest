...
import { Upload, Type, X, Loader2, Sparkles, Music2, Timer } from 'lucide-react';
...
const FilesPanel = ({
  ...
  // Sincronização automática só fica clicável depois que o usuário fizer
  // upload de música original E letra (arquivo ou colada manualmente)
  const hasLyrics = stanzaCount > 0 || Boolean(uploaded.letraArquivo) || Boolean(uploaded.letraManual);
  const canAutoSync = Boolean(mediaFiles?.musicaOriginal) && hasLyrics && stanzaCount > 0 && !isAutoSyncing;
  ...
  const handleAutoSync = async () => {
    if (!canAutoSync || anyBusy || !isAuthenticated || !stanzas.length) return;
    ...
    try {
      ...
      onNotify?.({
        ...
        message: '🎵 Estamos ouvindo a música para sincronizar sua letra automaticamente...',
      });

      // Envia os blocos EXATAMENTE como estão — a estrutura existente na
      // LenaVS é mantida. O Lyrics Aligner só descobre os tempos das palavras.
      const response = await api.post('/lyrics/auto-sync', {
        audioUrl: mediaFiles.musicaOriginal,
        blocks: stanzas.map((stanza) => ({ text: stanza.text })),
      });

      const timedBlocks = Array.isArray(response.data?.blocks) ? response.data.blocks : [];

      if (!timedBlocks.length) {
        throw new Error('O servidor não retornou os tempos dos blocos.');
      }

      // Aplica os tempos por índice — nenhum bloco é criado, excluído,
      // dividido, juntado ou reorganizado. Blocos sem tempo identificado
      // mantêm os tempos que já tinham. O editor continua mostrando
      // somente minutos e segundos (mm:ss).
      let syncedCount = 0;

      const syncedStanzas = stanzas.map((stanza, index) => {
        const block = timedBlocks[index];

        if (!block || block.startTime == null || block.endTime == null) {
          return stanza;
        }

        const startTime = formatFixedTimecode(block.startTime);
        const endTime = formatFixedTimecode(block.endTime);

        if (startTime === stanza.startTime && endTime === stanza.endTime) {
          return stanza;
        }
      ...
      if (!syncedCount) {
        onNotify?.({
          type: 'warning',
          title: 'Nenhum bloco sincronizado',
          message: 'O Lyrics Aligner não conseguiu identificar os tempos desta letra. Verifique se ela corresponde ao que é cantado e tente novamente.',
        });
        return;
      }

      onStanzasSynced?.(syncedStanzas);

      onNotify?.({
        type: 'success',
        title: '✨ Pronto!',
        message: 'Sua letra foi sincronizada automaticamente. Ajuste os tempos manualmente se quiser.',
      });
    } catch (error) {
      console.error('[FilesPanel] Erro na sincronização automática:', error);
      // Mensagem amigável — nunca exibe o erro técnico cru para o usuário.
      onNotify?.({
        type: 'error',
        title: 'Não foi possível sincronizar',
        message: 'Não conseguimos concluir a sincronização automática. Verifique se a letra corresponde ao que é cantado na música e tente novamente.',
      });
    } finally {
    ...
    : !mediaFiles?.musicaOriginal
      ...
      : 'Gera a versão sem voz com IA.'}
        ...
        <div className="auto-sync-wrapper">
          <button
            type="button"
            className={`auto-sync-btn ${canAutoSync ? 'unlocked' : 'locked'}`}
            onClick={handleAutoSync}
            disabled={!canAutoSync || anyBusy}
            title={
              !mediaFiles?.musicaOriginal
                ? 'Faça o upload da música original primeiro'
                : !hasLyrics
                  ? 'Faça o upload da letra primeiro'
                  : !stanzas.length
                    ? 'Nenhum bloco de letra encontrado para sincronizar'
                    : 'Alinha a letra com a música usando o Lyrics Aligner'
            }
          >
            {isAutoSyncing ? (
              <span className="auto-sync-btn-content">
                ...
                <span className="auto-sync-btn-note">os blocos da letra serão posicionados automaticamente</span>
              </span>
            ) : (
              <span className="auto-sync-btn-content">
                <span className="auto-sync-btn-main">
