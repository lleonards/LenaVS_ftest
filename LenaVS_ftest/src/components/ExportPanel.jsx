...
const ExportPanel = ({
  ...
  const waitForGeneration = async (response) => {
    ...
    // Polling com backoff exponencial: 3s → ~4s → ~5,5s → … (teto de 15s).
    // Em servidor de 1 vCPU / 2GB, consultar a cada 1,5s fixo derrubava o
    // backend durante a renderização. Agora a carga cai ~4x no mesmo período.
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
      ...
      delayMs = Math.min(15000, Math.round(delayMs * 1.35));
