...
### Compatibilidade com a fila de recursos do backend

- A chamada de sincronização automática pode permanecer aberta enquanto o
  backend aguarda a vaga única de CPU/RAM e executa o Lyrics Aligner.
- O timeout da API foi ampliado para 30 minutos, sem criar novas tentativas
  automáticas que poderiam duplicar uma tarefa pesada.
- Quando o backend responde `503` porque a fila está cheia, a interface mostra
  uma orientação para tentar novamente mais tarde, em vez de exibir um erro
  genérico.
